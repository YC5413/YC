...
CONFIG = {
    ...
    # Boost 行為
    'boost_epsilon': 0.40,          # boost 生效時的 epsilon 下限
    'boost_duration': 100,          # boost 持續的 episode 數
    'boost_cooldown': 40,          # boost 結束後的冷卻 episode 數
    ...
    'boost_disable_recent_flag_window': 100,  # 觀察最近 N 回合 flag
    'boost_disable_recent_flag_count': 10,   # 最近 N 回合 flag 達到此數量後永久停用 boost
    # =========================================================

    # ==================== Replay Bonus ====================
    'replay_bonus_enabled': True,       # 總開關，方便一鍵關閉做對照實驗
    'dz_bonus_window': 200,              # bonus 範圍：death zone ± 80 的 transition
    'dz_success_margin': 150,           # 成功穿越條件：episode_max_x >= death_zone + 250
    'dz_bonus_factor': 30,             # priority 溫和加權倍數
    'dz_bonus_max_age': 3000,           # 超過 3000 episodes 不再保留額外 bonus
    'dz_bonus_decay_gamma': 0.9998,      # 每增加 1 episode age，bonus 乘上 0.999
...
class DeathZoneBoost:
    ...
    def record_flag(self, x: float):
        """通關時更新 clear_x，是否停用 boost 交由近期 flag 表現判斷。"""
        self.cleared_once = True
        self.clear_x = float(x)
        self.flag_count += 1

    def maybe_disable_from_recent_flags(self, recent_flag_count: int, recent_window_size: int):
        """近期通關率夠穩定時才永久停用 boost。"""
        if self.boost_disabled_forever:
            return
        target_window = int(self.cfg.get('boost_disable_recent_flag_window', 100))
        target_count = int(self.cfg.get('boost_disable_recent_flag_count', 25))
        if recent_window_size >= target_window and recent_flag_count >= target_count:
...
class BonusAwareReplayBuffer:
    ...
    def memorize_with_metadata(self, state, action, reward, next_state, done,
        ...
        # 穩定版：只對新寫入的 bonus 樣本做一次溫和 priority 加權
        self._apply_insertion_bonus_once(is_dz_bonus=is_dz_bonus, bonus_factor=bonus_factor)

    def update_with_bonus(self) -> Optional[float]:
        """
        穩定版：不在每次 update 前反覆修改 priorities，
        避免同一批 bonus 樣本被重複乘上 factor。
        bonus 只在寫入 replay buffer 當下做一次溫和加權。
        """
        return self._original_update()

    def _apply_insertion_bonus_once(self, is_dz_bonus: bool, bonus_factor: float):
        """
        只在 transition 寫入 replay buffer 時，對新插入樣本做一次 priority 加權。
        這比每次 update 前重複乘 factor 更穩定。
        """
        ...
        if memory is None:
            return
        if not hasattr(memory, 'priorities') or not isinstance(memory.priorities, np.ndarray):
            return
        ...
        idx = (int(memory.position) - 1) % len(priorities)
        factor = max(1.0, float(bonus_factor))
        priorities[idx] *= factor
        self.last_batch_avg_bonus_factor = factor
...
def train():
    ...
    log_print(
        ...
        f"disable_recent={CONFIG['boost_disable_recent_flag_count']}/{CONFIG['boost_disable_recent_flag_window']}"
    )
    log_print(
    ...
    for episode in range(CONFIG['num_episodes']):
        ...
        while not done:
            ...
            # 通關 / 失敗統計口徑與 x_max_pos 對齊
            if pending_flag:
                dzb.record_flag(max(x_max_pos, x_now))
                pending_flag = False

            # done=True and not flag_obtained 代表 Mario 死亡或超時
            if done and not flag_obtained:
                dzb.record_death(x_max_pos)
        ...
        # 計算成功率
        recent_window_size = min(len(flag_obtained_history), CONFIG['boost_disable_recent_flag_window'])
        flag_success_count = sum(flag_obtained_history[-CONFIG['boost_disable_recent_flag_window']:]) if flag_obtained_history else 0
        total_flags = sum(flag_obtained_history) if flag_obtained_history else 0
        dzb.maybe_disable_from_recent_flags(flag_success_count, recent_window_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--boost_disable_recent_flag_window', type=int, default=None)
    parser.add_argument('--boost_disable_recent_flag_count', type=int, default=None)
    ...
    for key in ['death_zone_window', 'death_zone_min_samples', 'boost_spread_threshold',
                ...
                'boost_disable_recent_flag_window', 'boost_disable_recent_flag_count']:
